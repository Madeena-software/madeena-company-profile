...
" 2>/dev/null || true
    ...
    HTTP_STATUS=$(curl -sS -o /dev/null -w "%{http_code}" --max-time 15 http://127.0.0.1:8011 2>/dev/null || echo "000")
    if echo "$HTTP_STATUS" | grep -qE '^(200|301|302|307|308)$'; then
        _pass "http://127.0.0.1:8011 → HTTP $HTTP_STATUS"
    elif [ "$HTTP_STATUS" = "500" ]; then
        _pass "http://127.0.0.1:8011 → HTTP $HTTP_STATUS (app running, internal error expected in sim)"
    else
        _fail "http://127.0.0.1:8011 → HTTP $HTTP_STATUS (expected 2xx/3xx)"
    fi
...
# Start only db and app first — wait for them to be healthy
dc up -d db app
...
echo -e "  App:   ${CYAN}http://localhost:8011${RESET}"
echo ""
