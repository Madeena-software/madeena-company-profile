#!/bin/bash
# =============================================================================
# Madeena Standard — Hybrid Local Development Setup
# =============================================================================
# CUSTOMIZATION REQUIRED:
#   1. Replace {{APP_NAME}} with your app slug (lowercase, e.g. "pbu50")
#   2. Replace {{REQUIRED_PHP}} with your PHP version (e.g. "8.3")
#   3. Replace {{DB_PORT}} with your local MySQL port (e.g. "3306")
#   4. Update DB credentials if needed (defaults: user={{APP_NAME}}, pass=secret)
#
...
show_help() {
    cat <<'HELPTEXT'
madeena_cp — Hybrid Local Development Setup
...
Prerequisites:
  • PHP with extensions: pdo_mysql mbstring xml gd zip intl bcmath pcntl
  • Composer 2.x
  • Node.js 18+ and NPM
...
sync_local_env() {
    ...
    set_env "DB_DATABASE" "madeena_cp"
    set_env "DB_USERNAME" "madeena_cp"
...
# ─── HEADER ───────────────────────────────────────────────────────────────────
echo ""
echo -e "${BOLD}🚀  ${APP_NAME} — Hybrid Local Development Setup${RESET}"
echo "────────────────────────────────────────────────"
if [[ "${FRESH}" == "true" ]]; then
...
if [[ ! -f "${ENV_FILE}" ]] || [[ "${FRESH}" == "true" ]]; then
    ...
    info "Existing .env found; syncing local defaults"
fi
